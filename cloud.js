...
AV.Cloud.define('start-conv', function (req, res) {
  ...
  // 找到最闲的客服
  var query = new AV.Query('Staff').limit(20).ascending('concurrent');
  query.find().then(function (staffs) {
    return getFirstOnlineStaff(staffs);
  }).then(function (staff) {
    if (!staff) {
      var error = new Error('No staff online');
...
function getFirstOnlineStaff(staffs) {
  // 获取客服的在线状态
  return request({
    method: 'POST',
    uri: 'https://leancloud.cn/1.1/rtm/online',
    body: {
      peers: staffs.map(function (staff) {
        return staff.get('clientId');
      })
    },
    headers: {
      'X-LC-Id': process.env.LC_APP_ID,
    ...
    // 返回在线的最闲的客服
    var clientId = res.results[0];
    return staffs.reduce(function (prev, curr) {
      return curr.get('clientId') === clientId ? curr : prev;
    }, null);
  });
}
